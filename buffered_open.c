#include <C:\Users\Azrie\CLionProjects\os2.3\buffered_open.h>
#include <stdlib.h>
#include<stdio.h>
#include <string.h>
#include <signal.h>
...
buffered_file_t *buffered_open(const char *pathname, int flags, ...) {
    ...
    //if O_PREAPPEND is parts of the flags - preappend =1;
    if (flags&O_PREAPPEND) {
        bf->preappend = 1;
    }
    //set fd
    bf->flags&~O_PREAPPEND; //remove O_PREAPPEND from flags
    if (bf->flags&O_CREAT) {
        int mode = 0644;
        bf->fd = open(pathname,flags,mode);
    } else { bf->fd = open(pathname,flags); }
    //allocate memory for the buffers
    bf->read_buffer = (char*) calloc(1,BUFFER_SIZE );
    bf->write_buffer = (char*) calloc(1,BUFFER_SIZE );
...
ssize_t buffered_read(buffered_file_t *bf, void *buf, size_t count) {
    ...
    while(actuallyRead < count) {
        if (bf->read_buffer_pos >= bf->read_buffer_size || bf->read_buffer_size == 0) {
            int readFromFile = read(bf->fd,bf->read_buffer, count);
            if (readFromFile < 0) {
                return -1;  // Error
            }
        ...
        memcpy(buf + actuallyRead, bf->read_buffer ,toCopy);
